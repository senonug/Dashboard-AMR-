...
if uploaded_file:
    ...
    # Pastikan semua kolom numerik di-cast dengan benar
    num_cols = [
    ...
    result['Jumlah_Kriteria'] = indikator_df.sum(axis=1)
    top50 = result.sort_values(by='Jumlah_Kriteria', ascending=False).head(50)

    # Ringkasan
    st.metric("Total Data Berhasil di Analisis", len(df))
    st.metric("Total IDPEL di Analisis", df['LOCATION_CODE'].nunique())
    st.metric("Target Operasi Memenuhi Kriteria", sum(result['Jumlah_Kriteria'] > 0))
